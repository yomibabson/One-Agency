...
trait DCE_Trait_WP {
	...
	public static function get_user_value( $user_id = null, $field = 'display_name', $single = null ) {
		...
		if ( is_array( $metaValue ) ) {
			...
			if ( $single === true || count( $metaValue ) == 1 ) {
	...
	public static function get_term_value( $term = null, $field = 'name', $single = null ) {
		...
		if ( is_array( $termValue ) ) {
			...
			if ( $single === true || count( $termValue ) == 1 ) {
	...
	public static function get_post_css( $post_id = null, $theme = false ) {
		...
		$elementor_styles = array(
			...
			'dce-frontend-css' => DVE_PATH . DCE_Assets::$minifyCss,
		);
		if ( $theme ) {
	...
	public static function get_wp_query_args() {
		...
		//http://codex.wordpress.org/Class_Reference/WP_Query#Password_Parameters
			'has_password', //(bool) - available with Version 3.9
		...
		//http://codex.wordpress.org/Class_Reference/WP_Query#Pagination_Parameters
			...
			'posts_per_archive_page', //(int) - number of posts to show per page - on archive pages only. Over-rides showposts anposts_per_page on pages where is_archive() or is_search() would be true
			'nopaging', //(bool) - show all posts or use pagination. Default value is 'false', use paging.
			'paged', //(int) - number of page. Show the posts that would normally show up just on page X when usinthe "Older Entries" link.
		...
		//http://codex.wordpress.org/Class_Reference/WP_Query#Order_.26_Orderby_Parameters
			'order', //(string) - Designates the ascending or descending order of the 'orderby' parameter. Default to 'DESC'.
		//Possible Values:
		...
		//http://codex.wordpress.org/Class_Reference/WP_Query#Permission_Parameters
			'perm', //(string) Possible values are 'readable', 'editable'
		//////Caching Parameters
		...
		//NOTE Caching is a good thing. Setting these to false is generally not advised.
			...
			'no_found_rows', //(bool) Default is false. WordPress uses SQL_CALC_FOUND_ROWS in most queries in order to implement pagination. Even when you donтАЩt need pagination at all. By Setting this parameter to true you are telling wordPress not to count the total rows and reducing load on the DB. Pagination will NOT WORK when this parameter is set to true. For more information see: http://flavio.tordini.org/speed-up-wordpress-get_posts-and-query_posts-functions
		//////Search Parameter
